...
Imports MySql.Data.MySqlClient
    ...
    Imports System.IO
Public Class parts_OUT_Results
    ...
    Private Sub datagrid1_CellClick(sender As Object, e As DataGridViewCellEventArgs) Handles datagrid1.CellClick
        Try
            ...
            Dim cmdrefreshgrid As New MySqlCommand("SELECT dp.qrcode, dp.partno, pm.partname, dp.lotnumber, dp.qty
                                                FROM denso_parts dp
                                                JOIN denso_scanoperator ds ON dp.userout = ds.IDno
                                                JOIN denso_parts_masterlist pm ON dp.partno = pm.partno
                                                WHERE dp.dateout = '" & dtpicker.Value.ToString("yyyy-MM-dd") & "' 
                                                AND ds.Fullname = '" & cmbuser.Text & "' 
                                                AND dp.partno = '" & itempartcode & "' 
                                                AND dp.batchout = '" & cmbbatchin.Text & "'", con)
    ...
    Private Sub dtpicker_ValueChanged_1(sender As Object, e As EventArgs) Handles dtpicker.ValueChanged
        Try
            ...
            Dim cmdselect As New MySqlCommand("Select distinct ds.fullname FROM denso_parts dp
                                                INNER JOIN denso_scanoperator ds ON userout = IDno
                                                WHERE dateout='" & dtpicker.Value.ToString("yyyy-MM-dd") & "' ORDER BY ds.fullname ASC", con)
    ...
    Private Sub cmbuser_SelectedIndexChanged(sender As Object, e As EventArgs) Handles cmbuser.SelectedIndexChanged
        Try
            con.Close()
            con.Open()
            Dim cmdselect As New MySqlCommand("Select distinct dp.batchout FROM denso_parts dp
                                                JOIN denso_scanoperator ds ON ds.IDno = dp.userout
    ...
    Private Sub Guna2Button1_Click(sender As Object, e As EventArgs) Handles Guna2Button1.Click
        savetoPDF(datagrid2, "Parts Outgoing Report")
    End Sub
